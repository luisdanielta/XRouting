...
export const FailurePredictionSchema = z.object({
  riskLevel: FailureRiskLevel.describe('Risk level of failure (low, medium, high)'),
  estimatedFailureDate: z.coerce.date().nullable().describe('Predicted failure date if available'),
...
export const MaintenanceSchema = z.object({
  ...
  subcomponentsAffected: z.array(z.string()).default([]).describe('List of affected subcomponent IDs'),
  ...
  description: z.string().nullable().describe('Description of the maintenance performed'),
  nextMaintenance: z.coerce.date().nullable().describe('Scheduled next maintenance date'),
  failurePrediction: FailurePredictionSchema.nullable().describe('Prediction of potential future failures')
});
