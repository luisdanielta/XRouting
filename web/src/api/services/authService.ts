...
export class AuthService implements IAuthService {
    ...
    public async login(username: string, password: string): Promise<{ user: User; token: string }> {
        try {
            const response = await this.apiClient.request<{ token: string }>(E.auth.login, {
                method: "POST",
            ...
            const user = new User(decoded.name, decoded.username, decoded.email, true, decoded.role);
     ...
     */
    public async signup(username: string, email: string, password: string): Promise<{ message: string }> {
        try {
    ...
    public getUserFromToken(): User | null {
        ...
        try {
            const decoded = jwtDecode<DecodedToken>(token);
            return new User(decoded.name, decoded.username, decoded.email, true, decoded.role);
        } catch (error) {
            console.error("Error decoding token:", error);
            return null;
        }
...
export const authServiceInstance = new AuthService(ApiClient.getInstance(), TokenStorage);
